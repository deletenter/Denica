<?php
session_start();
$is_logged_in = isset($_SESSION['customer_id']);
$customer_name = $_SESSION['customer_name'] ?? '';
?>

<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <title>Denica - Your Cart</title>
    <link rel="stylesheet" href="style.css">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css">
</head>
<body>

    <header class="global-header">
        <div class="header-container">
            <a href="index.html" class="brand-identity">Denica</a>
            <nav class="main-nav">
                <a href="products.php">Perfumes</a>
                <a href="quiz/discovery.html">Discovery</a>
            </nav>
            <div class="header-utilities">
                <a href="cart.html" class="cart-link">
                    <i class="fas fa-shopping-bag"></i>
                    <span id="nav-cart-count" class="cart-count">1</span>
                </a>
                <?php if ($is_logged_in): ?>
                    <a href="userprofile.php" class="profile-link" style="display:flex; align-items:center; gap:8px;">
                        <i class="fas fa-user-circle"></i>
                        <span><?php echo htmlspecialchars($customer_name); ?></span>
                    </a>
                <?php else: ?>
                    <a href="login.php" class="profile-link" style="display:flex; align-items:center; gap:8px;">
                        <i class="fas fa-user-circle"></i>
                        <span>Login</span>
                    </a>
                <?php endif; ?>
            </div>
        </div>
    </header>

    <div id="empty-state" style="display: none; text-align: center; margin-top: 100px;">
        <h2>Your cart is empty</h2>
        <p style="color: #666; margin: 15px 0;">Looks like you haven't found your perfumes yet.</p>
        <a href="products.html" style="text-decoration: none;">
            <button class="btn-continue" style="max-width: 200px; padding: 15px 30px; background: black; color: white; border: none; cursor: pointer;">Start Shopping</button>
        </a>
    </div>

    <main class="cart-container" id="cart-main-content" style="margin-top: 40px;">
        <section class="cart-items"> <h1 style="font-size: 2rem; margin-bottom: 10px;">Your cart</h1>
            <p class="subtitle" style="margin-bottom: 30px;">Not ready to checkout? <a href="products.html" style="color:black;">Continue Shopping</a></p>

            <div class="cart-product cart-item"> <div class="img-box" style="width: 100px; height: 100px; background: #eee;">
                    <img src="perfume1.jpg" alt="Perfume" style="width:100%; height:100%; object-fit:cover; display:block;"> 
                </div> 
                <div class="product-details" style="flex: 1; margin-left: 20px;">
                    <h3>Medin Kirana</h3>
                    <p style="color: #666; font-size: 0.9rem;">Size: 15ML</p>
                    
                    <div style="margin: 10px 0;">
                        Quantity: 
                        <input type="number" class="qty-input" value="1" min="1" style="width: 60px; padding: 5px;">
                    </div>
                    
                    <p><strong>RM <span class="item-price">39.00</span></strong></p>
                    
                    <button class="remove-link" onclick="removeItem(this)" style="background: none; border: none; color: #888; text-decoration: underline; cursor: pointer; padding: 0;">Remove</button>
                </div>
            </div>

            <div class="order-info-accordion" style="margin-top: 40px; border-top: 1px solid #eee; padding-top: 20px;">
                <details open style="margin-bottom: 15px; cursor: pointer;">
                    <summary style="font-weight: bold;">Return Policy</summary>
                    <p style="margin-top: 10px; color: #666; font-size: 0.9rem;">Returns are accepted within 30 days of purchase.</p>
                </details>
                <details style="cursor: pointer;">
                    <summary style="font-weight: bold;">Shipping Options</summary>
                    <p style="margin-top: 10px; color: #666; font-size: 0.9rem;">Standard delivery takes 3-5 business days via JnT or Poslaju.</p>
                </details>
            </div>
        </section>

        <aside class="order-summary">
            <h3>Order Summary</h3>
            <input type="text" placeholder="Enter coupon code here" style="width: 100%; margin-bottom: 20px;">
            
            <div class="summary-line price-row">
                <span>Subtotal</span> 
                <span id="subtotal-display">RM 39.00</span>
            </div>
            <div class="summary-line price-row">
                <span>Shipping</span> 
                <span>Calculated at next step</span>
            </div>
            
            <div class="summary-line total-row">
                <span>Total</span> 
                <span id="total-display">RM 39.00</span>
            </div>

            <button class="btn-checkout" onclick="validateCheckout()">Continue to checkout</button>
        </aside>
    </main>

<script>
    // --- 1. MATH LOGIC ---
    function calculateCart() {
        let subtotal = 0;
        let totalQty = 0;
        
        const items = document.querySelectorAll('.cart-product');
        const emptyState = document.getElementById('empty-state');
        const mainContent = document.getElementById('cart-main-content');
        
        // Check if empty
        if (items.length === 0) {
            mainContent.style.display = 'none'; 
            emptyState.style.display = 'block'; 
            document.getElementById('nav-cart-count').innerText = "0";
            return; 
        }

        // Calculate Total
        items.forEach(item => {
            const priceElement = item.querySelector('.item-price');
            const qtyElement = item.querySelector('.qty-input');

            if (priceElement && qtyElement) {
                const price = parseFloat(priceElement.innerText);
                const quantity = parseInt(qtyElement.value);
                subtotal += price * quantity;
                totalQty += quantity;
            }
        });

        // Update Text
        document.getElementById('subtotal-display').innerText = 'RM ' + subtotal.toFixed(2);
        document.getElementById('total-display').innerText = 'RM ' + subtotal.toFixed(2);
        document.getElementById('nav-cart-count').innerText = totalQty;
    }

    function removeItem(btn) {
        if(confirm("Remove this item from cart?")) {
            btn.closest('.cart-product').remove();
            calculateCart(); 
        }
    }

    document.addEventListener('input', function (e) {
        if (e.target.classList.contains('qty-input')) {
            calculateCart();
        }
    });
    
    // --- 2. LOGIN CHECK LOGIC ---
    function validateCheckout() {
      // Inject PHP session value into JS
      const isLoggedIn = <?php echo $is_logged_in ? 'true' : 'false'; ?>;

      if (isLoggedIn) {
          // Go to your checkout page (change to checkout.php if you use PHP)
          window.location.href = 'checkout_info.html';
      } else {
          alert("Please log in to proceed to checkout.");
          // Optional: return user back to cart after login
          window.location.href = 'login.php?redirect=cart.php';
      }
  }

    // Run math on load
    window.onload = calculateCart;
</script>

</body>
</html>

